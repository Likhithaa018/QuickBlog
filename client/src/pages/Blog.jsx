...
const Blog = () => {
  const {id} = useParams() //useParams is helping us to get blog_id from the url

  const {axios} = useAppContext()

  const [data,setData] = useState(null) //stores current blog data
  const [comments,setComments] = useState([]) //stores comments

  const [name,setName] = useState('') //stores name for new comments
  const [content,setContent] = useState('') //stores content for new comments
      ...
      <div className='mx-5 max-w-5xl md:mx-auto my-10 mt-6'>
        ...
        <div className='max-w-3xl mx-auto'>
          ...
          gap-4 max-w-lg'>
            ...
            <textarea onChange={(e)=> setContent(e.target.value)} value={content} placeholder='Comment' className='w-full
            p-2 border border-gray-300 rounded outline-none h-48' required></textarea>
            <button type='submit' className='bg-primary text-white rounded p-2
            px-8 hover-102 transition-all cursor-pointer'>Submit</button>
          </form>
        </div>
        {/* Share options */}
        <div>
